 ...
 * @param {Tag[][]} tags - The tags to be attached to each file.
...
async function prepareFiles(
	files: File[],
	tags: Tag[][],
	ephemeralSigner: ArweaveSigner,
): Promise<Map<string, DataItem>> {
	const items: [string, DataItem][] = await Promise.all(
		files.map(async (file, index) => {
			return [file.name, await prepareFile(file, tags[index], ephemeralSigner)];
		}),
	);
 ...
 * @param {Tag[]} fileTags - The tags to be attached to the file.
 ...
 */
async function prepareFile(file: File, fileTags: Tag[], ephemeralSigner: ArweaveSigner): Promise<DataItem> {
	const defaultTags = [{ name: "Content-Type", value: file.type }];
	const allTags = [...defaultTags, ...fileTags];
...
/**
 * Uploads the bundled data to the Irys
 *
 ...
 *
 * @param {File[]} files - The files to be uploaded.
 * @param {Tag[]} tags - An array of tag arrays, one per file.
 * @returns {Promise<string>} - The transaction ID of the upload.
 */
const fundAndUploadNestedBundle = async (files: File[], tags: Tag[][]): Promise<string[]> => {
	try {
		const ephemeralSigner = new ArweaveSigner(await Arweave.crypto.generateJWK());
		const preppedFiles = await prepareFiles(files, tags, ephemeralSigner);
